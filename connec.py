...
with open("MainMenu.html","w") as html_file:
    html_file.write(html_content)
print("Html file created successfully !!")
time.sleep(2)
webbrowser.open_new_tab("MainMenu.html")
list_foods = []                   
list_drinks = []   
list_services = []                                                
...
def def_full_file_reader():                                                                        
    ...
    while i <= (len(list_foods) - 1): 
        if '-R' in list_foods[i]:
            list_foods[i] = str(list_foods[i][:list_foods[i].index('-R') - 1]) + ' ' * (20 - (list_foods[i].index('-R') - 1)) + str(list_foods[i][list_foods[i].index('-R'):])
        i += 1

    i = 0
    while i <= (len(list_drinks) - 1):
        if '-R' in list_drinks[i]:
            list_drinks[i] = str(list_drinks[i][:list_drinks[i].index('-R') - 1]) + ' ' * (20 - (list_drinks[i].index('-R') - 1)) + str(list_drinks[i][list_drinks[i].index('-R'):])
        i += 1

    i = 0
    while i <= (len(list_services) - 1):
        if '-R' in list_services[i]:
            list_services[i] = str(list_services[i][:list_services[i].index('-R') - 1]) + ' ' * (20 - (list_services[i].index('-R') - 1)) + str(list_services[i][list_services[i].index('-R'):])
        i += 1
...
def def_file_sorter(): 
    ...
    while i < len(list_foods):
        list_item_price[i] = float(list_foods[i][int(list_foods[i].index("-R") + 2):]) 
        i += 1

    i = 0
    while i < len(list_drinks):
        list_item_price[40 + i] = float(list_drinks[i][int(list_drinks[i].index("R") + 2):]) 
        i += 1

    i = 0
    while i < len(list_services):
        list_item_price[80 + i] = float(list_services[i][int(list_services[i].index("-R") + 2):]) 
        i += 1
